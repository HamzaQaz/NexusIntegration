...
console.log(chalk.magentaBright('CLIENT'), chalk.yellow(`Logging in...`));
client.login(client.config.TOKEN);
client.on('ready', function () {
    console.log(chalk.magentaBright('CLIENT'), chalk.green(`Logged in as ${client.user.tag}!`));
...
async function InteractionHandler(interaction, type) {
    ...
    try {
        ...
        if (component.owner) {
            if (interaction.user.id !== "1247276436415320140") return await interaction.reply({ content: `⚠️ Only bot owners can use this command!`, ephemeral: true });
        }
